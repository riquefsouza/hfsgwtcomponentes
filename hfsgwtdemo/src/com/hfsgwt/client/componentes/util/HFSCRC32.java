...
public class HFSCRC32 {

	static final long table[] = { 0x00000000L, 0x77073096L, 0xEE0E612CL,
	...
	public synchronized static String CryptCRC32(String QueTexto) {
		long result;

		if (QueTexto.length() > 0) {
			result = CalcCRC32(QueTexto);
			result = ~result;
	...
	private static byte[] toBytes(char[] inputChars) {
	...
	private static long CalcCRC32(String QueTexto) {

		//byte p[] = QueTexto.getBytes();
		byte p[] = toBytes(QueTexto.toCharArray());
		long mynCRC32 = 0xFFFFFFFFL;

		for (int i = 0; i < (p.length); i++) {
			// CRCvalue := (CRCvalue SHR 8) XOR aTable[ q^ XOR (CRCvalue AND
			// $000000FF) ];
			mynCRC32 = (mynCRC32 >> 8)
					^ table[(int) (p[i] ^ (mynCRC32 & 0x000000FF))];
		}
